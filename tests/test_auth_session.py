...
def test_auth_session_defaults_to_long_lived_rolling_session():
    app = _make_app()
    try:
        assert app.config["PERMANENT_SESSION_LIFETIME"] == timedelta(days=3650)
        assert app.config["REMEMBER_COOKIE_DURATION"] == timedelta(days=3650)
